(function () {
	...
	var baseExtension = createCommonjsModule(function (module, exports) {
	  ...
	  var BaseExtension = function () {
	    function BaseExtension() {
	      (0, _classCallCheck3.default)(this, BaseExtension);
	    }

	    (0, _createClass3.default)(BaseExtension, [{
	      key: "bootstrap",
	...
	var _objectKeysInternal = function (object, names) {
	  ...
	  for (key in O) if (key != IE_PROTO$1) _has(O, key) && result.push(key); // Don't enum bug & hidden keys
	  ...
	  exports.default = {
	    ...
	    gunify: function gunify(key) {
	      ...
	      function applyResult(val) {
	        // metadata
	        node._['>'][val.field] = val.state; // relation
	...
	var adapterError = createCommonjsModule(function (module, exports) {
	  ...
	  var AdapterError = function (_Error) {
	    (0, _inherits3.default)(AdapterError, _Error);

	    function AdapterError(message, code, fileName, lineNumber) {
	      (0, _classCallCheck3.default)(this, AdapterError);

	      var _this = (0, _possibleConstructorReturn3.default)(this, (AdapterError.__proto__ || (0, _getPrototypeOf2.default)(AdapterError)).call(this, message, fileName, lineNumber));
	...
	unwrapExports(adapterError);

	var adapterContext = createCommonjsModule(function (module, exports) {
	  ...
	  var _adapterError2 = _interopRequireDefault(adapterError);
	...
	unwrapExports(adapterContext);

	var baseMixin = createCommonjsModule(function (module, exports) {
	  ...
	  var BaseMixin = function BaseMixin(context) {
	    var _this2 = this;

	    var keys = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
	    (0, _classCallCheck3.default)(this, BaseMixin);

	    var _this = this;

	    keys.forEach(function (propName) {
	      var orig = context[propName];
	      ...
	      if (orig && typeof orig === 'function') {
	        ...
	        context[propName] = _this2[propName];
	      }
	    });
	  };
	...
	var union = function (Gun, vertex, node, opt) {
	  if (!node || !node._) {
	    return;
	  }
	...
	var gun = createCommonjsModule(function (module) {
	  ...
	  (function () {
	    ...
	    USE(function (module) {
	      ...
	      var Type = {}; //Type.fns = Type.fn = {is: function(fn){ return (!!fn && fn instanceof Function) }}
	    ...
	    USE(function (module) {
	      ...
	      module.exports = function onto(tag, arg, as) {
	        ...
	        var u,
	            tag = (this.tag || (this.tag = {}))[tag] || (this.tag[tag] = {
	          tag: tag,
	          to: onto._ = {
	            next: function (arg) {
	              var tmp;
	    ...
	    USE(function (module) {
	      ...
	      (function () {
	        ...
	        function merge(node, soul) {
	          ...
	          if (!ctx.and) {
	            ...
	            cat.on('node', function (m) {
	              ...
	              if (m !== ctx.map[m.get]) {
	                return;
	              } // filter out events not from this context!
	              ...
	              if (obj_map(ctx.souls, function (v) {
	                ...
	                return;
	              } // if flag still outstanding, keep waiting.
	              ...
	              ctx.c = 1; // failsafe for only being called once per context.
	        ...
	        Gun.on.get = function (msg, gun) {
	          ...
	          if (has) {
	            ...
	            node = Gun.state.to(node, has); // If we have a key in-memory, do we really need to fetch?
	      ...
	      var state_lex = Gun.state.lex,
	          _soul = Gun.val.link._,
	          _has = '.',
	          node_ = Gun.node._,
	          rel_is = Gun.val.link.is;
	      var empty = {},
	          u;

	      console.only = function (i, s) {
	        return console.only.i && i === console.only.i && console.only.i++ && (console.log.apply(console, arguments) || s);
	      };
	    ...
	    USE(function (module) {
	      ...
	      function input(msg) {
	        ...
	        if (cat.has && at !== cat) {
	          ...
	          if (at.ack) {
	            cat.ack = at.ack; //cat.ack = cat.ack || at.ack;
	          }
	        }
	        ...
	        if (u === change) {
	          ...
	          if (cat.soul) {
	            return;
	          } // TODO: BUG, I believee the fresh input refactor caught an edge case that a `gun.get('soul').get('key')` that points to a soul that doesn't exist will not trigger val/get etc.
	        ...
	        if (!(rel = Gun.val.link.is(change))) {
	          if (Gun.val.is(change)) {
	            if (cat.has || cat.soul) {
	              ...
	              (cat.map || (cat.map = {}))[at.id] = cat.map[at.id] || {
	                at: at
	          ...
	          tmp = (root.stop || {})[at.id]; //if(tmp && tmp[cat.id]){ } else {
	        ...
	        var was = root.stop;
	        tmp = root.stop || {};
	        tmp = tmp[at.id] || (tmp[at.id] = {}); //if(tmp[cat.id]){ return }

	        tmp.is = tmp.is || at.put;
	        tmp[cat.id] = at.put || true; //if(root.stop){
	      ...
	      function relate(at, msg, from, rel) {
	        ...
	        tmp = from.id ? (at.map || (at.map = {}))[from.id] = at.map[from.id] || {
	          at: from
	        } : {};
	        ...
	        if (at.pass) {
	          Gun.obj.map(at.map, function (tmp) {
	            tmp.pass = true;
	          });
	          obj_del(at, 'pass');
	        }
	      ...
	      function not(at, msg) {
	        ...
	        if (u === tmp && Gun.val.link.is(at.put)) {
	          return;
	        } // This prevents the very first call of a thing from triggering a "clean up" call. // TODO: link.is(at.put) || !val.is(at.put) ?
	    ...
	    USE(function (module) {
	      ...
	      function use(msg) {
	        ...
	        if ((tmp = root.mum) && at.id) {
	          // TODO: can we delete mum entirely now?
	        ...
	        if (as.soul || root === gun) {
	          if (!obj_is(as.data)) {
	            (as.ack || noop).call(as, as.out = {
	              err: Gun.log("Data saved to the root level of the graph must be a node (an object), not a", typeof as.data, 'of "' + as.data + '"!')
	            });
	    ...
	    USE(function (module) {
	      ...
	      function val(msg, eve, to) {
	        ...
	        if (!to && u === data && eve.ack <= (opt.acks || Object.keys(at.root.opt.peers).length)) {
	          return;
	        }

	        if (!to && (u === data || at.soul || at.link || link && !(0 < link.ack)) || u === data && (tmp = Object.keys(at.root.opt.peers).length) && !to && (link || at).ack < tmp) {
	      ...
	      */
	      ...
	      Gun.on('create', function (root) {
	        ...
	        if (false === opt.localStorage) {
	          return ev.next(root);
	        } // we want offline resynce queue regardless! // actually, this doesn't help, per @go1dfish 's observation. Disabling for now, will need better solution later.


	        opt.prefix = opt.file || 'gun/';
	        var gap = Gun.obj.ify(store.getItem('gap/' + opt.prefix)) || {};
	        var empty = Gun.obj.empty,
	            id,
	            to;
	 // add re-sync command.
	        ...
	        root.on('out', function (msg) {
	          if (msg.lS) {
	            return;
	          } // TODO: for IndexedDB and others, shouldn't send to peers ACKs to our own GETs.
	        ...
	        function ack(ack) {
	          // TODO: This is experimental, not sure if we should keep this type of event hook.
	          if (ack.err || !ack.ok) {
	            return;
	          }
	    ...
	    USE(function (module) {
	      ...
	      function Mesh(root) {
	        ...
	        mesh.hear = function (raw, peer) {
	          ...
	          if ('{' != raw[2]) {
	            mesh.hear.d += raw.length || 0;
	            ++mesh.hear.c;
	          } // STATS! // ugh, stupid double JSON encoding
	          ...
	          if ('[' === tmp) {
	            ...
	            (function go() {
	              var S;
	              LOG && (S = +new Date()); // STATS!

	              var m,
	                  c = 100; // hardcoded for now?
	          ...
	          if ('{' === tmp || Type.obj.is(raw) && (msg = raw)) {
	            ...
	            if (msg.DBG_s) {
	              opt.log(+new Date() - msg.DBG_s, 'to hear', id);
	            }
	            ...
	            if (hash && (tmp = msg['@'] || msg.get && id)) {
	              // Reduces backward daisy in case varying hashes at different daisy depths are the same.
	              if (dup.check(tmp + hash)) {
	                return;
	              }
	        ...
	        (function () {
	          ...
	          mesh.say = function (msg, peer) {
	            ...
	            var id, hash, tmp, raw;
	            var S;
	            LOG && (S = +new Date()); //msg.DBG_s = msg.DBG_s || +new Date;
	            ...
	            if (!peer) {
	              peer = (tmp = dup.s[msg['@']]) && (tmp = tmp.it) && (tmp = tmp._) && (tmp = tmp.via);
	            }
	        ...
	        (function () {
	          mesh.raw = function (msg) {
	            ...
	            if (!msg.dam) {
	              var i = 0,
	                  to = [];
	              Type.obj.map(opt.peers, function (p) {
	                to.push(p.url || p.pid || p.id);

	                if (++i > 9) {
	                  return true;
	                } // limit server, fast fix, improve later! // For "tower" peer, MUST include 6 surrounding ids.
	      ...
	      (function () {
	        ...
	        function sort(k, v) {
	          ...
	          Type.obj.map(Object.keys(v).sort(), map, {
	            to: tmp = {},
	            on: v
	          });
	          return tmp;
	...
	var baseAdapter = createCommonjsModule(function (module, exports) {
	  ...
	  var _baseExtension2 = _interopRequireDefault(baseExtension);

	  var _adapterContext2 = _interopRequireDefault(adapterContext);

	  var _baseMixin2 = _interopRequireDefault(baseMixin);
	  ...
	  var BaseAdapter = function (_BaseExtension) {
	    ...
	    function BaseAdapter(adapter) {
	      ...
	      _this2.outerContext = _adapterContext2.default.make(_this2); // Bind the three adapter methods to the `this` context
	      ...
	      _this2._put = adapter.put ? adapter.put.bind(_this2.outerContext) : _util2.default.noop; // Bind all adapter methods to the outer context
	      ...
	      _this2.__dedupIds = {}; // Bind context for read and write methods
	      // These receive context from Gun when they are called

	      _this2._read = _this2._read.bind(_this2);
	      _this2._write = _this2._write.bind(_this2); // Apply Mixins
	    ...
	    (0, _createClass3.default)(BaseAdapter, [{
	      ...
	      value: function _write(context) {
	        var delta = context.put,
	            dedupId = context['#']; // Filter out returns from the `get` requests
	...
	unwrapExports(baseAdapter);

	var deltaAdapter = createCommonjsModule(function (module, exports) {
	  ...
	  var _baseAdapter2 = _interopRequireDefault(baseAdapter);
	  ...
	  var DeltaAdapter = function (_BaseAdapter) {
	    (0, _inherits3.default)(DeltaAdapter, _BaseAdapter);

	    function DeltaAdapter() {
	      (0, _classCallCheck3.default)(this, DeltaAdapter);
	      return (0, _possibleConstructorReturn3.default)(this, (DeltaAdapter.__proto__ || (0, _getPrototypeOf2.default)(DeltaAdapter)).apply(this, arguments));
	    }

	    (0, _createClass3.default)(DeltaAdapter, [{
	      key: 'read',
	...
	unwrapExports(deltaAdapter);

	var keyValAdapter = createCommonjsModule(function (module, exports) {
	  ...
	  var KeyValAdapter = function (_BaseAdapter) {
	    ...
	    (0, _createClass3.default)(KeyValAdapter, [{
	      ...
	      value: function write(delta, done) {
	        ...
	        var keys = (0, _keys2.default)(delta); // Batch together key:val writes

	        var batch = []; // Iterate through each node in the delta

	        keys.forEach(function (key) {
	          var nodeDelta = delta[key];
	          var conflictState = nodeDelta._['>']; // Iterate through each field in the node delta

	          (0, _keys2.default)(nodeDelta).map(function (field) {
	            // Ignore meta info
	            if (field !== '_') {
	              var state = conflictState[field]; // base node
	...
	var nodeAdapter = createCommonjsModule(function (module, exports) {
	...
	unwrapExports(nodeAdapter);

	var ProfilerMixin_1 = createCommonjsModule(function (module, exports) {
	  ...
	  var ProfilerMixin = function (_BaseMixin) {
	    ...
	    (0, _createClass3.default)(ProfilerMixin, [{
	      ...
	      value: function opt(context, next) {
	        ...
	        console.warn('You are using the Gun-Flint profiler mixin. This is intended to profile your adapter during development and should not be deployed into production environments.'); // Update every minute
	...
	var ResultStreamMixin_1 = createCommonjsModule(function (module, exports) {
	  ...
	  var _events2 = _interopRequireDefault(EventEmitter);
	...
	unwrapExports(ResultStreamMixin_1);

	var dist = createCommonjsModule(function (module) {

	  var _baseExtension2 = _interopRequireDefault(baseExtension);

	  var _deltaAdapter2 = _interopRequireDefault(deltaAdapter);

	  var _keyValAdapter2 = _interopRequireDefault(keyValAdapter);

	  var _nodeAdapter2 = _interopRequireDefault(nodeAdapter);

	  var _baseMixin2 = _interopRequireDefault(baseMixin);

	  var _ProfilerMixin2 = _interopRequireDefault(ProfilerMixin_1);

	  var _ResultStreamMixin2 = _interopRequireDefault(ResultStreamMixin_1);
	  ...
	  var flint = {
	    register: function register() {
	      var extension = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null; // if (!extension || !(extension instanceof BaseExtension)) {
